...
void ShuttleManager::init(ros::NodeHandle nh)
{
	...
	subProductChange = nh.subscribe("/planiftest/ShuttlePChange", 10, &ShuttleManager::productChangeCallBack, this);
	subDestinationChange = nh.subscribe("/planiftest/ShuttleDChange", 10, &ShuttleManager::destinationChangeCallBack, this);
	subZoneChange = nh.subscribe("/planiftest/ShuttleZChange", 10, &ShuttleManager::zoneChangeCallBack, this);
	subAddShuttle = nh.subscribe("/planiftest/AddShuttle", 10, &ShuttleManager::addShuttleCallBack, this);
	subDelShuttle = nh.subscribe("/planiftest/DelShuttle", 10, &ShuttleManager::delShuttleCallBack, this);
}
...
bool ShuttleManager::getShuttleStatus(shuttles::srvGetShuttleStatus::Request &req, shuttles::srvGetShuttleStatus::Response &res)
{
	...
	if (it != ShuttlesMap.end()) 		// Vrai si l'itérateur n'est pas hors de la liste, donc qu'on a bien trouvé un shuttle avec l'handler du message
	{
		...
		ROS_INFO("serviceGetShuttleStatus est en cours d'execution");
...
bool ShuttleManager::getEmptyShuttles(shuttles::srvGetEmptyShuttles::Request &req, shuttles::srvGetEmptyShuttles::Response &res)
{
 	...
 	for (it=ShuttlesMap.begin(); it!=ShuttlesMap.end(); ++it)	// On parcours l'ensemble de la collection
	{
		...
		{
			ROS_INFO("serviceGetEmptyShuttles a trouve une navette vide!");
...
void ShuttleManager::addShuttleCallBack(const shuttles::msgShuttleCreate::ConstPtr& msg)
{
	...
	ROS_INFO("Creation navette %s", n.c_str());
	...
	{
    		ROS_WARN("shuttleManager : A shuttle with this handler already exists in internal memory !"); 
  	} 
}
...
{		
	...
	{
		ROS_WARN("ShuttleManager : Navette non trouve pour suppression ! ");
	}
}
...
void ShuttleManager::productChangeCallBack(const shuttles::msgShuttleChange::ConstPtr& msg)
{
	...
	{
		ROS_WARN("ShuttleManager : Navette non trouve pour changement de produit ! ");
	}
}
...
void ShuttleManager::destinationChangeCallBack(const shuttles::msgShuttleChange::ConstPtr& msg)
{
	...
	{
		ROS_WARN("ShuttleManager : Navette non trouve pour changement de destination ! ");
	}
...
void ShuttleManager::zoneChangeCallBack(const shuttles::msgShuttleChange::ConstPtr& msg)
{
	...
	{
		ROS_WARN("ShuttleManager : Navette non trouve pour changement de zone ! ");
	}
