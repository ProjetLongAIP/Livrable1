...
{
    circle(imageSensor, cv::Point(88, 345), 5, cv::Scalar(0,255*SensorState.CPI[1],255-255*SensorState.CPI[1]), -1, 8 );
    circle(imageSensor, cv::Point(172, 345), 5, cv::Scalar(0,255*SensorState.CPI[2],255-255*SensorState.CPI[2]), -1, 8 );
    circle(imageSensor, cv::Point(860, 344), 5, cv::Scalar(0,255*SensorState.CPI[3],255-255*SensorState.CPI[3]), -1, 8 );
    circle(imageSensor, cv::Point(945, 346), 5, cv::Scalar(0,255*SensorState.CPI[4],255-255*SensorState.CPI[4]), -1, 8 );
    circle(imageSensor, cv::Point(908, 22), 5, cv::Scalar(0,255*SensorState.CPI[5],255-255*SensorState.CPI[5]), -1, 8 );
    circle(imageSensor, cv::Point(825, 22), 5, cv::Scalar(0,255*SensorState.CPI[6],255-255*SensorState.CPI[6]), -1, 8 );
    circle(imageSensor, cv::Point(169, 20), 5, cv::Scalar(0,255*SensorState.CPI[7],255-255*SensorState.CPI[7]), -1, 8 );
    circle(imageSensor, cv::Point(83, 20), 5, cv::Scalar(0,255*SensorState.CPI[8],255-255*SensorState.CPI[8]), -1, 8 );
...
void UI::DrawStopSensorImg(commande_locale::Msg_SensorState SensorState)
{
    ...
    circle(imageSensor, cv::Point(115, 345), 5, cv::Scalar(0,255*SensorState.PS[2],255-255*SensorState.PS[2]), -1, 8 );
    circle(imageSensor, cv::Point(198, 345), 5, cv::Scalar(0,255*SensorState.PS[3],255-255*SensorState.PS[3]), -1, 8 );
    circle(imageSensor, cv::Point(241, 230), 5, cv::Scalar(0,255*SensorState.PS[4],255-255*SensorState.PS[4]), -1, 8 );
    circle(imageSensor, cv::Point(248, 342), 5, cv::Scalar(0,255*SensorState.PS[5],255-255*SensorState.PS[5]), -1, 8 );
    circle(imageSensor, cv::Point(326, 230), 5, cv::Scalar(0,255*SensorState.PS[6],255-255*SensorState.PS[6]), -1, 8 );
    circle(imageSensor, cv::Point(572, 231), 5, cv::Scalar(0,255*SensorState.PS[7],255-255*SensorState.PS[7]), -1, 8 );
    circle(imageSensor, cv::Point(700, 231), 5, cv::Scalar(0,255*SensorState.PS[8],255-255*SensorState.PS[8]), -1, 8 );
    circle(imageSensor, cv::Point(834, 345), 5, cv::Scalar(0,255*SensorState.PS[9],255-255*SensorState.PS[9]), -1, 8 );
    circle(imageSensor, cv::Point(922, 347), 5, cv::Scalar(0,255*SensorState.PS[10],255-255*SensorState.PS[10]), -1, 8 );
    circle(imageSensor, cv::Point(934, 232), 5, cv::Scalar(0,255*SensorState.PS[11],255-255*SensorState.PS[11]), -1, 8 );
    circle(imageSensor, cv::Point(992, 339), 5, cv::Scalar(0,255*SensorState.PS[12],255-255*SensorState.PS[12]), -1, 8 );
    circle(imageSensor, cv::Point(995, 189),5, cv::Scalar(0,255*SensorState.PS[13],255-255*SensorState.PS[13]), -1, 8 );
    circle(imageSensor, cv::Point(934, 21), 5, cv::Scalar(0,255*SensorState.PS[14],255-255*SensorState.PS[14]), -1, 8 );
    circle(imageSensor, cv::Point(852, 22), 5, cv::Scalar(0,255*SensorState.PS[15],255-255*SensorState.PS[15]), -1, 8 );
    circle(imageSensor, cv::Point(783, 135), 5, cv::Scalar(0,255*SensorState.PS[16],255-255*SensorState.PS[16]), -1, 8 );
    circle(imageSensor, cv::Point(770, 28), 5, cv::Scalar(0,255*SensorState.PS[17],255-255*SensorState.PS[17]), -1, 8 );
    circle(imageSensor, cv::Point(701, 136), 5, cv::Scalar(0,255*SensorState.PS[18],255-255*SensorState.PS[18]), -1, 8 );
    circle(imageSensor, cv::Point(454, 135), 5, cv::Scalar(0,255*SensorState.PS[19],255-255*SensorState.PS[19]), -1, 8 );
    circle(imageSensor, cv::Point(321, 135), 5, cv::Scalar(0,255*SensorState.PS[20],255-255*SensorState.PS[20]), -1, 8 );
    circle(imageSensor, cv::Point(195, 20), 5, cv::Scalar(0,255*SensorState.PS[21],255-255*SensorState.PS[21]), -1, 8 );
    circle(imageSensor, cv::Point(109, 20), 5, cv::Scalar(0,255*SensorState.PS[22],255-255*SensorState.PS[22]), -1, 8 );
    circle(imageSensor, cv::Point(66, 135), 5, cv::Scalar(0,255*SensorState.PS[23],255-255*SensorState.PS[23]), -1, 8 );
    circle(imageSensor, cv::Point(29, 26), 5, cv::Scalar(0,255*SensorState.PS[24],255-255*SensorState.PS[24]), -1, 8 );
...
void UI::onMouse_internal( int event, int x, int y)
{
	...
	switch(event) {
		case cv::EVENT_LBUTTONDOWN :
   			if(y>545 && y < 570 && x>400 && x<510 && mode!= 1) {
					playButton_Down.copyTo(imageTot.rowRange(545,570).colRange(400,510));
					pauseButton.copyTo(imageTot.rowRange(545,570).colRange(550,660));
					VREPController->play();
					mode = 1; }
    		else if(y>545 && y < 570 && x>550 && x<660 && mode !=0) {
					playButton.copyTo(imageTot.rowRange(545,570).colRange(400,510));
					pauseButton_Down.copyTo(imageTot.rowRange(545,570).colRange(550,660));
					VREPController->pause();
					mode = 0; }
		else if(y>545 && y < 570 && x>700 && x<810 && mode!= 0) {
					shuttleButton_Down.copyTo(imageTot.rowRange(545,570).colRange(700,810));
					VREPController->shuttle('F');
					 }
		break;
		case cv::EVENT_MOUSEMOVE :
   			if(y>545 && y < 570 && x>400 && x<510 && mode!= 1)
					playButton_On.copyTo(imageTot.rowRange(545,570).colRange(400,510));
				else if (mode!= 1)
					playButton.copyTo(imageTot.rowRange(545,570).colRange(400,510));
    		if(y>545 && y < 570 && x>550 && x<660 && mode !=0)
					pauseButton_On.copyTo(imageTot.rowRange(545,570).colRange(550,660));
				else if (mode!= 0)
					pauseButton.copyTo(imageTot.rowRange(545,570).colRange(550,660));

		if(y>545 && y < 570 && x>700 && x<810 && mode!= 0)
					shuttleButton_On.copyTo(imageTot.rowRange(545,570).colRange(700,810));
				else if (mode!= 0)
					shuttleButton.copyTo(imageTot.rowRange(545,570).colRange(700,810));
		break;
	}
}
...
void UI::init(ros::NodeHandle nh){
	...
	shuttleButton = cv::imread("img/ShuttleButton.png",CV_LOAD_IMAGE_COLOR);
	shuttleButton_Down = cv::imread("img/ShuttleButton_Down.png",CV_LOAD_IMAGE_COLOR);
	shuttleButton_On = cv::imread("img/ShuttleButton_On.png",CV_LOAD_IMAGE_COLOR);
